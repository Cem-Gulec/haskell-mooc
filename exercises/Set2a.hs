...
substring i j s = if i == j 
                  then ""
                  else reverse(take (j - i) (reverse (take j s)))
...
isPalindrome str = if str == "" || length str == 1
                   then True
...
palindromify' :: Int -> String -> String
palindromify' n s
    | isPalindrome (substring n (length s - n) s) = substring n (length s - n) s
    | otherwise = palindromify' (n+1) s

palindromify :: String -> String
palindromify s = if s == "" || length s == 2
                 then ""
                 else if length s == 1
                 then s
                 else palindromify' 1 s
