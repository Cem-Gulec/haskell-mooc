...
rgb Green = [0, 1, 0]
rgb Blue = [0, 0, 1]
rgb (Mix col1 col2) = mixColors (rgb col1) (rgb col2)
rgb (Invert col) = invertColors (rgb col)

-- Helper function to mix two colors
mixColors :: [Double] -> [Double] -> [Double]
mixColors [] [] = []
mixColors (r1:rest1) (r2:rest2) = ((r1 + r2) / 2) : mixColors rest1 rest2

-- Helper function to invert a color
invertColors :: [Double] -> [Double]
invertColors [] = []
invertColors (r:rest) = (1 - r) : invertColors rest
...
fromList [] = Empty
fromList ((key, value):pairs) = Pair key value (fromList pairs)
...
prettyPrint bin = if null bits then "0" else bits
  where
    bits = prettyPrint' bin ""

    prettyPrint' :: Bin -> String -> String
    prettyPrint' End acc = acc
    prettyPrint' (O b) acc = prettyPrint' b ('0' : acc)
    prettyPrint' (I b) acc = prettyPrint' b ('1' : acc)
