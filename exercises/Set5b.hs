...
isOrdered (Node root left right) =
  allValues (< root) left && allValues (> root) right && isOrdered left && isOrdered right
...
search val tree = searchHelper [] val tree

searchHelper :: Eq a => [Step] -> a -> Tree a -> Maybe [Step]
searchHelper path _ Empty = Nothing  
searchHelper path val (Node x left right)
  | val == x = Just path  
  | otherwise =
    case (searchHelper (path ++ [StepL]) val left) of
      Just newPath -> Just newPath
      Nothing -> case (searchHelper (path ++ [StepR]) val right) of
                   Just newPath -> Just newPath
                   Nothing -> Nothing