...
count element = foldr (\x acc -> if x == element then acc + 1 else acc) 0
...
inBoth xs ys = foldr (\x acc -> if x `elem` ysList then x : acc else acc) [] xs
  where
    ysList = foldl (\acc y -> y : acc) [] ys
