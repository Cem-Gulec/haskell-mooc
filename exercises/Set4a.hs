...
longest (x:xs) = findLongest x xs
  where
    findLongest longestList [] = longestList
    findLongest longestList (y:ys)
      | length y > length longestList = findLongest y ys
      | length y == length longestList && head y < head longestList = findLongest y ys
      | otherwise = findLongest longestList ys
...
freqs xs = foldr (\x -> Map.alter (updateCount x) x) Map.empty xs
  where
    updateCount :: a -> Maybe Int -> Maybe Int
    updateCount _ (Just count) = Just (count + 1)
    updateCount _ _ = Just 1
...
maxIndex arr =
  let indexValuePairs = assocs arr
  in fst $ maximumBy (\(_, a) (_, b) -> compare a b) indexValuePairs