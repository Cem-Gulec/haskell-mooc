...
shout :: T.Text -> T.Text
shout text = T.unwords $ zipWith capitalizeIfEven [0..] (T.words text)
  where
    capitalizeIfEven idx word
      | even idx = T.toUpper word
      | otherwise = word
...
longestRepeat text
  | T.null text = 0  
  | otherwise = go (T.tail text) (T.head text) 1 1
  where
    go :: T.Text -> Char -> Int -> Int -> Int
    go t prevChar currentLength maxLength
      | T.null t = max currentLength maxLength
      | char == prevChar = go (T.tail t) char (currentLength + 1) maxLength
      | otherwise = go (T.tail t) char 1 (max currentLength maxLength)
      where
        char = T.head t
...
takeStrict n lazyText = T.take (fromIntegral n) (TL.toStrict (TL.take n lazyText))
...
byteRange bs
  ...
  where
    byteList = B.unpack bs  
    maxByte = maximum byteList
    minByte = minimum byteList
