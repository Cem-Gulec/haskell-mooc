...
checkNumber (forename, surname)
  | any isDigit forename || any isDigit surname = Nothing
...
selectSum xs indices = do
  selectedElements <- traverse (safeIndex xs) indices
  return (sum selectedElements)
...
countAndLog predicate xs = do
  let (selected, remaining) = partition predicate xs
  mapM_ (logElement . show) selected
  return (length selected)
  where
    logElement :: String -> Logger ()
    logElement = msg
...
balance accountName = BankOp balanceHelper
  where balanceHelper bank = (balanceOfAccount accountName bank, bank)

balanceOfAccount :: String -> Bank -> Int
balanceOfAccount accountName (Bank accounts) =
...
rob from to = balance from +> (\balanceFrom -> withdrawOp from balanceFrom +> depositOp to)
...
count x = modify $ \state ->
  let (found, rest) = partition (\(a, _) -> a == x) state
  in case found of
       [] -> (x, 1) : state  
       [(a, n)] -> (a, n + 1) : rest  
       _ -> error "Multiple usage of same element"
