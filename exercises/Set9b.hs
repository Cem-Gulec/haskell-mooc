...
prettyPrint n coords = concatMap processRow [1..n]
  where
    processRow i = concatMap (processCell i) [1..n] ++ "\n"
    processCell i j = if (i,j) `elem` coords then ['Q'] else ['.']
...
sameDiag (i, j) (k, l) = (i - k) == (j - l) || (k - i) == (l - j)
...
danger candidate stack = any (threatens candidate) stack
  where
    threatens c q = sameRow c q || sameCol c q || sameDiag c q || sameAntidiag c q
...
prettyPrint2 n stack = concatMap processRow [1 .. n]
  where
    processRow i = concatMap (processCell i) [1 .. n] ++ "\n"
    processCell i j
      | (i,j) `elem` stack = "Q"
...
fixFirst n (q:qs) = go q
  where
    go (i, j)
      | j > n     = Nothing
      | danger (i, j) qs = go (i, j + 1)
...
backtrack [] = []
backtrack [_] = []
backtrack ((i, j):q:qs) = (nextRow q, nextCol q) : qs
  where
    nextRow (a, _) = a
    nextCol (_, b) = b + 1
